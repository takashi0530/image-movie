...
test("画像をアップロードすると動画が生成されプレビュー＆ダウンロードできる", async ({
  ...
  // BGM の選択肢がサーバから読み込まれ、チップで1曲選べる
  // （静的な「おまかせ」チップは常在なので、サーバ由来の曲名で待つ）
  const bossa = page.getByRole("button", { name: "ボサノバ" });
  await expect(bossa).toBeVisible();
  await bossa.click();
  await expect(bossa).toHaveClass(/selected/);
  // 選択曲のプレビューとクレジット表記（CC BY）が表示される
  await expect(page.locator(".track-preview audio")).toBeVisible();
  await expect(page.locator(".track-credit")).toContainText("Kevin MacLeod");
